...
path_images_from_camera = "service/kernel/facialrecognition/Dlib_face_recognition_from_camera/data/data_faces_from_camera/"
...
def return_features_mean_personX(img_rd_set):
    ...
    if features_list_personX:
        features_mean_personX = np.array(features_list_personX, dtype=object).mean(
            axis=0
        )
    else:
...
def main(img_rd_set, person_name):
    ...
    for i in range(csv_rd.shape[0]):
        if person_name == csv_rd.iloc[i][0]:
            ...
            for j in range(1, csv_rd.shape[1]):
                csv_rd.iloc[i, j] = (
                    0.5 * csv_rd.iloc[i][j] + 0.5 * features_mean_personX[j - 1]
                )
            break
    ...
    with open(
        ...
        for index in range(csv_rd.shape[0]):
            ...
            logging.info("\n")
        logging.info(
            "%s的人脸数据存入 / Save the features of face registered into: service/kernel/facialrecognition/Dlib_face_recognition_from_camera/data/features_all.csv",
            person_name,
        )


if __name__ == "__main__":
    path1 = "service/kernel/facialrecognition/Dlib_face_recognition_from_camera/data/data_faces_from_camera/person_1_hwy/img_face_1.jpg"
